...
import { PLATFORM_CLASS_META, type Layout, type PlatformDef, type Pt, type SidingDef } from '../../shared/layout'

function platYAt(pf: PlatformDef, x: number): number {
  const yl = pf.yL ?? pf.y, yr = pf.yR ?? pf.y
  return yl + (yr - yl) * ((x - pf.leftX) / ((pf.rightX - pf.leftX) || 1))
}
...
function sidingRefY(L: Layout): number { return L.platforms.length ? Math.max(...L.platforms.map(p => p.y)) : 700 }
// park route: platform -> Anschlussweiche on the bottom platform -> down the lead -> into the siding
function parkPoly(pf: { centerX: number, y: number }, sd: SidingDef, L: Layout): Pt[] {
  const j = { x: sd.leftX, y: sidingRefY(L) }
  return [{ x: pf.centerX, y: pf.y }, j, { x: sd.leftX, y: sd.y }, { x: sd.centerX, y: sd.y }]
}
function trainPos(t: TrainView): Pt | null {
  ...
  if ((t.state === 'PARKING' || t.state === 'RETRIEVING') && t.platform != null && t.sidingIndex != null) {
    ...
    if (pf && sd) { const poly = parkPoly(pf, sd, L); return alongPoly(t.state === 'PARKING' ? poly : [...poly].reverse(), t.progress) }
  }
...
function drawBody(cv: HTMLCanvasElement) {
  ...
  // platforms: thin running line (full span, angled) + thicker platform edge (variable length)
  for (const pf of L.platforms) {
    const yl = pf.yL ?? pf.y, yr = pf.yR ?? pf.y
    line(ctx, { x: pf.leftX, y: yl }, { x: pf.rightX, y: yr }, '#3c454e', 3)
    if (pf.openL) diamond(ctx, { x: pf.leftX, y: yl }, 4, '#7a8590'); else buffer(ctx, pf.leftX, yl)
    if (pf.openR) diamond(ctx, { x: pf.rightX, y: yr }, 4, '#7a8590'); else buffer(ctx, pf.rightX, yr)
    const bl = pf.barL ?? pf.leftX, br = pf.barR ?? pf.rightX
    line(ctx, { x: bl, y: platYAt(pf, bl) }, { x: br, y: platYAt(pf, br) }, '#6b7682', 8)
    if (snap.platformDisabled[pf.index - 1]) hatch(ctx, bl, br, pf.y)
    text(ctx, `Gl ${pf.index}`, pf.leftX - 14, yl - 9, '#8b97a3', 21, 'right')
    const m = PLATFORM_CLASS_META[pf.cls]; text(ctx, m.tag, pf.leftX - 14, yl + 11, m.color, 15, 'right')
  }

  // sidings (Abstellgleise) — connected to the bottom platform via Anschlussweiche + lead
  const sRefY = sidingRefY(L)
  for (const sd of L.sidings) {
    line(ctx, { x: sd.leftX, y: sRefY }, { x: sd.leftX, y: sd.y }, '#3c454e', 3) // lead track up to the throat
    diamond(ctx, { x: sd.leftX, y: sRefY }, 4, '#7a8590') // Anschlussweiche
    line(ctx, { x: sd.leftX, y: sd.y }, { x: sd.rightX, y: sd.y }, '#4a5560', 6) // the siding itself
    buffer(ctx, sd.rightX, sd.y) // dead end
    text(ctx, `Abst ${sd.index}`, sd.rightX + 12, sd.y, '#6b7682', 15, 'left')
    const occ = inStation.find(t => t.sidingIndex === sd.index && t.state === 'PARKED')
    if (occ) line(ctx, { x: sd.leftX + 10, y: sd.y }, { x: sd.rightX - 10, y: sd.y }, '#7a6cff', 14)
  }
  ...
  for (const t of inStation) {
    ...
    const poly = parkPoly(pf, sd, L); for (let i = 0; i < poly.length - 1; i++) line(ctx, poly[i]!, poly[i + 1]!, '#7a6cff', 3)
  }
  ctx.setLineDash([])
  ...
  for (const pf of L.platforms) {
    ...
    const bl = pf.barL ?? pf.leftX, br = pf.barR ?? pf.rightX
    line(ctx, { x: bl + 6, y: platYAt(pf, bl + 6) }, { x: br - 6, y: platYAt(pf, br - 6) }, color, 16)
  }

  // preview route (the route the hovered menu option would set)
  if (props.previewPoly && props.previewPoly.length > 1) {
    const pv = props.previewPoly
    for (let i = 0; i < pv.length - 1; i++) line(ctx, pv[i]!, pv[i + 1]!, '#7df0ff', 6)
  }
